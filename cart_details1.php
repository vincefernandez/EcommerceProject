	...
	if(isset($_SESSION['user'])){
		if(isset($_SESSION['cart'])){
			foreach($_SESSION['cart'] as $row){
				...
				$crow = $stmt->fetch();
				if($crow['numrows'] < 1){
					$stmt = $conn->prepare("INSERT INTO orders (user_id, product_id, quantity,Sizes) VALUES (:user_id, :product_id, :quantity, :Sizes)");
					$stmt->execute(['user_id'=>$user['id'], 'product_id'=>$row['productid'], 'quantity'=>$row['quantity'], 'Sizes'=>$row['Sizes']]);
				}
				else{
					$stmt = $conn->prepare("UPDATE orders SET quantity=:quantity WHERE user_id=:user_id AND product_id=:product_id");
